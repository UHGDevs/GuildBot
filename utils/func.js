...
function getMilestones(first, second) {
...
function getSlayerLvl(xp, slayer) {
  //return 0
...
function getPitPrestige(xp) {
  var xps = [1.1, 1.2]
}

function getSwExpLeft(xp) {
	var xps = [0, 20, 70, 150, 250, 500, 1000, 2000, 3500, 6000, 10000, 15000]
...
function formatNumber(number, g="", max=2){
  if (!Number(number)) return `${g}${number}${g}`
  var options = {minimumFractionDigits: 0, maximumFractionDigits: max};
  if (g=="") return Number(number).toLocaleString('en', options)
  else return `${g}${Number(number).toLocaleString('en', options)}${g}`
}

function ratio(n1=0, n2=0, n3=2) {
...
function toTimeHours(minutes) {
  hours = minutes / 60
  playtime = `${Math.floor(Number(hours))}h`
  return playtime
}

function getRankedPosition(pos) {
	if (pos == 0) return ""
...
function f(number, max=2){
  ...
  let options = {minimumFractionDigits: 0, maximumFractionDigits: max};
...
function getArena(setup) {
  if (setup === null || setup === undefined) return
  else {
    return setup.toLowerCase()
        .replace(/_/gi, " ")
...
// async function db(name="bwserver", col="stats", type="get", query = {}, novy={}) {
//   let result;
//   let update = {$set:novy}
//   try {
//     await mongo.connect();
//     const database = mongo.db(name);
//     const collection = database.collection(col);
//     if (type=="post") await collection.insertOne(query);
//     else if (type=="delete") await collection.deleteOne(query);
//     else if (type=="update") await collection.updateOne(query, update);
//     else if (type=="stats") result = await collection.stats();
//     if (!result) result = await collection.find(query).toArray();
//   } finally {
//     await mongo.close();
//   }
//   return result
